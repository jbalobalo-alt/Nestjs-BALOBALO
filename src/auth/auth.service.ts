...
export class AuthService {
  ...
  async validateUser(username: string, pass: string) {
    ...
    if (valid) return { id: user.id, username, role: user.role };
    return null;
  }
  ...
  async login(user: any) {
    ...
    // Store refresh token using separate secret so you can revoke access by changing refresh secret
  ...
  async refreshTokens(refreshToken: string) {
    try {
      ...
      if (!user) throw new UnauthorizedException('Invalid refreh token');
      // check stored token matches
      const store = await this.usersService.findById(decoded.sub);
      ...
      // Instead of repeated calls, we use method that fetches refresh token
      ...
      const accessToken = this.jwtService.sign(payload);
      const newRefresh = jwt.sign (payload, process.env.JWT_REFRESH_TOKEN_SECRET || 'refresh', {
        expiresIn: process.env.REFRESH_TOKEN_EXPIRES_IN || '7d',
      });
